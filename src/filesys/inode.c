...
struct inode_disk
{
  block_sector start;   /* First data sector. */
...
struct inode
{
  ...
  block_sector sector;    /* Sector number of disk location. */
...
bool inode_create(block_sector sector, off_t length, bool is_dir)
{
...
struct inode *
inode_open(block_sector sector)
{
  struct list_elem *e;
  struct inode *inode;
...
off_t inode_read_at(struct inode *inode, void *buffer_, off_t size, off_t offset)
{
  ...
  while (size > 0)
  {
    ...
    block_sector sector_idx = byte_to_sector(inode, offset);
