...
struct block
{
  ...
  block_sector size;    /* Size in sectors. */
...
/* Verifies that SECTOR is a valid offset within BLOCK.
   Panics if not. */
static void
check_sector(struct block *block, block_sector sector)
{
...
void block_read(struct block *block, block_sector sector, void *buffer)
{
...
/* Write sector SECTOR to BLOCK from BUFFER, which must contain
   ...
   Internally synchronizes accesses to block devices, so external
   per-block device locking is unneeded. */
void block_write(struct block *block, block_sector sector, const void *buffer)
{
  check_sector(block, sector);
