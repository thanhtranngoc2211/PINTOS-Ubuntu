...
static void select_sector(struct ata_disk *, block_sector);
...
static void
ide_read(void *d_, block_sector sec_no, void *buffer)
{
...
/* Write sector SEC_NO to disk D from BUFFER, which must contain
   ...
   per-disk locking is unneeded. */
static void
ide_write(void *d_, block_sector sec_no, const void *buffer)
{
  struct ata_disk *d = d_;
  struct channel *c = d->channel;
  lock_acquire(&c->lock);
...
/* Selects device D, waiting for it to become ready, and then
   ...
   use LBA mode.) */
static void
select_sector(struct ata_disk *d, block_sector sec_no)
{
  struct channel *c = d->channel;
